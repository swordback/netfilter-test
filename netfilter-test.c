...
int _strstr(unsigned char* origin, int origin_len, unsigned char* find, int find_len) {
	int flag = 0;
	...
	*/
	if (tar_shost == 0) {
		if (_strstr(buf, size, str, strlen(str))) {
			tar_shost = ipp->ip_shost;
			return 1;
		}
	}
	else {
		if (tar_shost == ipp->ip_shost) {
			return 1;
		}
	}
	return 0;
